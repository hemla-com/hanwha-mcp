...
export async function setStorageConfig(input: z.infer<typeof setStorageConfigSchema>): Promise<string> {
  ...
  if (input.overwrite !== undefined) params["OverWrite"] = String(input.overwrite);
  if (input.autoDeleteEnable !== undefined) params["AutoDeleteEnable"] = String(input.autoDeleteEnable);
